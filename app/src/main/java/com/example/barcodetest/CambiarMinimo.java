...
public class CambiarMinimo extends AsyncTask<MainActivity, Void, Boolean> {
    ...
    protected Boolean doInBackground(MainActivity... mainActivities) {
        ...
        if(main.nuevo_min != null && !main.consulta.trim().isEmpty()){
            try {
                ...
                String CONSULTA = "UPDATE articulo SET invMin=? WHERE (clave=? OR claveAlterna=?) AND tipo!=2";
                PreparedStatement consultaPreparada = conexionBD.prepareStatement(CONSULTA);
                consultaPreparada.setString(1,main.nuevo_min.toString());
    ...
    protected void onPostExecute(Boolean cambiado) {
        ...
        if (cambiado){
            main.label_minimo.setText(main.input_minimo.getText().toString());
            Toast toast = Toast.makeText(main, "Se ha actualizado el inventario MINIMO correctamente",Toast.LENGTH_LONG);
